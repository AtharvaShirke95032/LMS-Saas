...
export async function POST(req:Request){
    const { combinedText, companionId } = await req.json();
    if(!combinedText && !companionId) return NextResponse.json({ error: "Text is required" }, { status: 400 });
    try {
        const notesGen = generateNote(combinedText,companionId);
